...
test("terminal keeps only a small technical scrollback", () => {
  assert.match(appSource, /const TERMINAL_SCROLLBACK_LINES = 120;/);
  assert.match(appSource, /scrollback: TERMINAL_SCROLLBACK_LINES/);
});

test("terminal viewport is compact without resizing the game pty", () => {
  assert.match(appSource, /const TERMINAL_COLS = 120;/);
  assert.match(appSource, /const TERMINAL_ROWS = 48;/);
